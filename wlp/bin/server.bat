...
:serverEnvDefaults
  ...
  if "!WLP_OUTPUT_DIR:,=!" == "!WLP_OUTPUT_DIR!" (
    set SERVER_IBM_JAVA_OPTIONS=-Xshareclasses:name=liberty-%%u,nonfatal,cacheDir="%WLP_OUTPUT_DIR%\.classCache" -XX:ShareClassesEnableBCI -Xscmx60m -Xscmaxaot4m !IBM_JAVA_OPTIONS!
  ) else (
...
:serverEnvAndJVMOptions
  ...
  set RC=0
  if exist "%SERVER_CONFIG_DIR%\jvm.options" (
    call:readJVMOptions "%SERVER_CONFIG_DIR%\jvm.options"
  ) else if exist "%WLP_INSTALL_DIR%\etc\jvm.options" (
    call:readJVMOptions "%WLP_INSTALL_DIR%\etc\jvm.options"
  )
goto:eof
